# PlayLite

**27 browser games. No downloads. No sign-up. Just play.**

🔗 **[play-lite.vercel.app](https://play-lite.vercel.app)**

---

## Games

| Category | Games |
|---|---|
| Arcade | Snake, Tetris, Breakout, Flappy Bird, Pong |
| Puzzle | 2048, Memory, Sudoku, Minesweeper, Number Guess, Math Challenge |
| Word | Hangman, Typing Speed, Word Guess, Word Scramble |
| Strategy | Tic Tac Toe, Connect Four, Rock Paper Scissors |
| Reflex | Reaction Test, Color Match, Whack-a-Mole, Bubble Pop |
| Memory | Simon Says |
| IQ | Stroop Test, Tower of Hanoi, N-Back, Number Sequences |

## Dev

```bash
npm run dev   # serves at localhost:3000
```

No build step. Pure ES modules — edit and refresh.
